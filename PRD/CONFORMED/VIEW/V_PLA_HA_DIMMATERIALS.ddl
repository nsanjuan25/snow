create or replace view V_PLA_HA_DIMMATERIALS(
	PLANTCODE,
	MATERIALCODE,
	PLANTKEY,
	BUYERKEY,
...
SELECT 
     ...
     , coalesce(  PPP.ProfitCenter,VPP.ProfitCenterCode, 'UNDF') as PROFITCENTERCODE
     ...
     , ifnull(try_to_number(sp.MLPERIODCURRENTSTDCOSTESTIMATE) ,0) as MLPERIODCURRENTSTDCOSTESTIMATE
     , ifnull(try_to_number(sp.MLFISCALYEARCURRENTSTDCOSTESTIMATE ),0) as MLFISCALYEARCURRENTSTDCOSTESTIMATE
...
/*   , IFNULL(GL.GLAccountNo,'') as GLAccountnumber
	 ...
	 , '' as GLDescription                                   
FROM   
       conformed.PLA_ha_site_Part sp                                                          
LEFT OUTER JOIN 
       sap.DimPlants P 
	       on sp.SiteCode = P.PlantCode
           AND P.DataSourceKey = 4
    LEFT OUTER JOIN conformed.PLA_PlantCode_ProfitCenter PPP                                              
        ON SP.SITECODE = PPP.PLANTCODE
    LEFT OUTER JOIN conformed.v_PLA_ProfitCenter_Mapping VPP
        ON SP.SITECODE = VPP.PLANTCODE
        AND SP.SITEPARTCODE = VPP.MATERIALCODE
                                              
LEFT OUTER JOIN 
       sap.DimBuyers B 
	       on  sp.buyercode = B.BuyerCode
           and b.datasourcekey = 4
LEFT OUTER JOIN 
       sap.DimPlanners Pl 
	       on  sp.plannercode = Pl.plannercode
           and pl.DataSourceKey = 4
LEFT OUTER JOIN 
       sap.dimMaterialGroups MG 
	       on  sp.materialgroupcode = MG.materialgroupcode
           and MG.DataSourcekey = 4   
/*
LEFT OUTER JOIN 
       sap.DimLotSizes dls 
	       on sp.LotSizeCode = dls.LotSizeCode */
LEFT OUTER JOIN 
       conformed.PLA_ha_mara m 
...
LEFT OUTER JOIN 
          (
              ...
              FROM
                  conformed.PLA_ha_site_Part sp
                  LEFT OUTER JOIN conformed.PLA_ha_tvkwz t ON t.werks = sp.sitecode
                  LEFT OUTER JOIN conformed.PLA_ha_mvke mv ON t.vkorg = mv.vkorg
...
/*      LEFT OUTER JOIN
            (
              ...
              FROM 
                  conformed.V_PLA_HA_DIMCUSTOMERMATERIALS
              ) CM 
...
/*      LEFT OUTER JOiN Sap.dimStrategicPurchasing dsp
    ...
    LEFT OUTER JOIN  conformed.PLA_ha_mbew mb 
    ...
    (
        ...
        from
            conformed.PLA_ha_mvke mvke
            INNER JOIN sap.DimPlants P on mvke.Vkorg = P.salesorganizationcode
/*            LEFT OUTER JOIN conformed.PLA_ha_tvm2t tvm2t 
                ON mvke.Mvgr2  = tvm2t.mvgr2
                AND tvm2t.spras = 'E'
            LEFT OUTER JOIN conformed.PLA_ha_tvm4t as tvm4t
                ON mvke.Mvgr4 = tvm4t.MVGR4
                AND tvm4t.SPRAS = 'E'
            LEFT OUTER JOIN conformed.PLA_ha_tvm5t as tvm5t                        
 ...
 /*   
    ...
    (
        ...
        FROM
            conformed.PLA_ha_t030 t030
            LEFT OUTER JOIN conformed.PLA_ha_skat skat on t030.konts = skat.saknr
        WHERE 
            skat.ktopl     = '0010'
            and skat.spras = 'E'
            and t030.ktopl = '0010'
