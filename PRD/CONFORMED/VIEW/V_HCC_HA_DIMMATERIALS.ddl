create or replace view V_HCC_HA_DIMMATERIALS(
	PLANTCODE,
	MATERIALCODE,
	PLANTKEY,
	BUYERKEY,
...
*/                            
  SELECT 
     ...
     , case when try_to_number(sp.MATERIALGROUPCODE) is null then 'string' ELSE 'numeric' END as matgroupisnumeric                        
     ...
     , ifnull(sp.minimumlotsize ,0) as minimumlotsize 
     ...
     , ifnull(sp.SAFETYINSTOCK, 0) as SAFETYINSTOCK
     ...
     , IFNULL( sp.AUTOPORESCHEDULESTRATEGY, '')  as AUTOPORESCHEDULESTRATEGY
     , IFNULL( sp.GENITEMCAT,'' ) as GENERALITEMCATEGORY
     , IFNULL( sp.QAUSAGE,'' ) as QAUSAGE
     , sp.FRMW_EXTRACTED_ON
     , sp.SNFLK_UPDATEDON                        
     , IFNULL( cm.customermaterialcode, '' ) as customermaterialcode
     , ifnull( m.zeinr,'' ) as documentnumber
     , ifnull( m.mtart,'' ) as materialtype
     , ifnull(sp5.matgroup5,'') materialgroupcode5
     , ifnull(sptstd.Description,'Undefined Part Type') as MaterialTypeDescription
     , ifnull(sptstd.MakeBuy,'') as MakeBuy
     , ifnull(sptstd.SiteParttypeStdCode,'UNDEFINED') as MaterialTypeStandardCode
     ...
     , IfNULL(sow.sourcingownertypecode ,'UNDEFINED') as sourcingownertypecode
     , IfNULL(sow.SourcingOwnerTypeStandardCode,'UNDEFINED')  as SourcingOwnerTypeStandardCode
     , IfNULL(sow.SourcingOwnerTypeDescription, 'UNDEFINED') as  SourcingOwnerTypeDescription
     , IfNULL(sow.controlledby ,'') as controlledby 
     , '' as salesorganizationcode          -- IFNULL(sa.salesorganizationcode,'')  as salesorganizationcode
     , '' as SalesMaterialGroup2            -- IFNULL(sa.SalesMaterialGroup2,'')  as SalesMaterialGroup2
     , '' as SalesMaterialGroup2Description -- IFNULL(sa.SalesMaterialGroup2Description,'') as SalesMaterialGroup2Description
     , '' as SalesMaterialGroup4            -- IFNULL(sa.SalesMaterialGroup4,'')  as SalesMaterialGroup4
     , '' as SalesMaterialGroup4Description -- IFNULL(sa.SalesMaterialGroup4Description,'') as SalesMaterialGroup4Description
     , '' as SalesMaterialGroup5            -- IFNULL(sa.SalesMaterialGroup5,'')  as SalesMaterialGroup5
     , '' as SalesMaterialGroup5Description -- IFNULL(sa.SalesMaterialGroup5Description,'') as SalesMaterialGroup5Description      
     
     , IFNULL(mb.bklas,'')  as ValuationClass
     , IFNULL(P.LocalCurrencyCode,'') AS CURRENCYCODE     
     , '' as GLAccountnumber        -- IFNULL(GL.GLAccountNo,'') as GLAccountnumber   -- needs skat table that is missing in S3
     , '' as GLDescription          -- IFNULL(GL.GLLong,'') as GLDescription     -- needs skat table that is missing in S3
  FROM   
      conformed.HCC_HA_SITE_PART sp    
      LEFT OUTER JOIN sap.DimPlants P on sp.SiteCode = P.PlantCode
          AND P.DataSourceKey = 2
      LEFT OUTER JOIN sap.DimBuyers B on sp.buyercode = B.BuyerCode
          and b.datasourcekey = 2
      LEFT OUTER JOIN sap.DimPlanners Pl on sp.plannercode = Pl.plannercode
          and pl.DataSourceKey = 2
      LEFT OUTER JOIN sap.dimMaterialGroups MG on sp.materialgroupcode = MG.materialgroupcode
          and MG.DataSourcekey = 2   
      LEFT OUTER JOIN sap.DimLotSizes dls on sp.LotSizeCode = dls.LotSizeCode
      LEFT OUTER JOIN conformed.hcc_ha_mara m on sp.sitepartcode = m.matnr
      LEFT OUTER JOIN 
          (
              SELECT 
                  sitepartcode,
              ...
              FROM
                  conformed.HCC_HA_SITE_PART sp
                  LEFT OUTER JOIN conformed.hcc_ha_tvkwz t ON t.werks = sp.sitecode
                  LEFT OUTER JOIN conformed.hcc_ha_mvke mv ON t.vkorg = mv.vkorg
...
/* BELOW Tables does not exists in Hadoop/APS so commenting out code, this may be revisit after golive        
    ...
    (
        ...
        from
            conformed.hcc_ha_mvke mvke
            INNER JOIN sap.DimPlants P on mvke.Vkorg = P.salesorganizationcode
            LEFT OUTER JOIN conformed.ecc_ha_tvm2t tvm2t 
                ON mvke.Mvgr2  = tvm2t.mvgr2
                AND tvm2t.spras = 'E'
            LEFT OUTER JOIN conformed.ecc_ha_tvm4t as tvm4t
                ON mvke.Mvgr4 = tvm4t.MVGR4
                AND tvm4t.SPRAS = 'E'
            LEFT OUTER JOIN conformed.ecc_ha_tvm5t as tvm5t                        
    ...
    (
        ...
        FROM
            conformed.hcc_ha_t030 t030
            LEFT OUTER JOIN conformed.hcc_ha_skat skat on t030.konts = skat.saknr
        WHERE 
            skat.ktopl     = '0010'
            and skat.spras = 'E'
            and t030.ktopl = '0010'
