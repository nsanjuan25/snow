create or replace view V_ECC_HA_FACTPURCHASEORDERS(
...
SELECT
    ...
    , pos.CURRENCYCODE
    , pos.DELETEFLAG 
    , iff( E.rcvprn is null , 'FALSE', 'TRUE' ) as SISEDIFLAG 
    ...
    --, 0 UNITPRICEBASEUOM  --hardcoded because we do not have MARC **********
    ...
    , pos.NETORDER 
    , pos.EXCHANGERATE
    , '' as PROFITCENTER2    --**** missing tables for PROFITCENTER2 this should come for Coupa Project
    , pos.POReminder1
    , pos.POReminder2
...
FROM
    (
      ...
      FROM
                  ...
                  from 
                      (
                          ...
                          from 
                              ...
                              inner join conformed.ecc_ha_eina i
          ...
          LEFT JOIN conformed.ecc_ha_tcurx TCURX 
              on ekko.waers = TCURX.currkey        

          LEFT OUTER JOIN conformed.ecc_ha_zmimac_poxref zz
    ...
    LEFT OUTER JOIN 
        ...
        AND DER.DataSourceKey = 1
    
    LEFT OUTER JOIN sap.DimCurrencyDecimalPlaces AS DCDP 
        ON   DCDP.CurrencyCode = pos.CurrencyCode 
	    AND DCDP.ExponentFactor > 0         
        AND DCDP.DataSourceKey = 1

    LEFT OUTER JOIN (select  DISTINCT rcvprn from  conformed.ecc_ha_EDP13 WHERE mestyp IN ('DELINS', 'ORDCHG', 'ORDERS', 'ZSM') ) e ON pos.SupplierCorporationDSCode = e.rcvprn
