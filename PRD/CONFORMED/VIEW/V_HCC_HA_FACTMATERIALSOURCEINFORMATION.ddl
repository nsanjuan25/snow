create or replace view V_HCC_HA_FACTMATERIALSOURCEINFORMATION(
	DATASOURCEKEY,
	SITECODE,
	SITEPARTCODE,
...
( 
	...
	( 
		SELECT 
			...
			FROM 
				CONFORMED.HCC_HA_EINE EINE 
				LEFT JOIN CONFORMED.HCC_HA_EINA EINA ON eine.infnr = EINA.infnr 
				LEFT JOIN CONFORMED.HCC_HA_TCURX tcurx ON eine.waers = tcurx.currkey 
			WHERE 
...
LEFT JOIN CONFORMED.HCC_HA_LFA1 LFA1 ON EINA_res.LIFNR = LFA1.LIFNR 
LEFT JOIN
( --> Comment convert the 2 rows to columns using MAX and Group BY 
	...
	( 
		...
		FROM 
			CONFORMED.HCC_HA_A017 A017 
			INNER JOIN CONFORMED.HCC_HA_KONP KONP ON A017.knumh = konp.knumh 
				AND A017.kappl = konp.kappl 
				AND A017.Kschl = konp.kschl 
				AND A017.kappl = 'M'
				AND A017.knumh > '0' 
				AND konp.loevm_ko = ''  --> comment Is initial 
			LEFT JOIN CONFORMED.HCC_HA_TCURX tcurx ON konp.konwa = tcurx.currkey 
		WHERE 
