create or replace view V_PUSHOUTS(
	CURRENTQUARTER,
	NEXTQUARTER,
	CURRENTFYQUARTER,
	PLANTKEY,
...
(
            ...
            ON PO.DataSourceKey = MRP.DataSourceKey
               ...
               AND PO.PURCHASEDOCUMENTTYPEKEY not in
                   (
                       ...
                       WHERE PURCHASEDOCUMENTTYPECODE in ( 'LPA', 'ZSM', 'LP', 'ZLP' )
                   )
    ...
    WHERE 
         CAST(MRP.snapshotdate AS DATE) = (SELECT MAX(datekey) FROM public.DIMDATES WHERE DAYNUMBERWEEK = 1 AND DATEKEY <= (CURRENT_DATE - 21 ))
          ...
          AND SP.Makebuy = 'Buy'
          AND MRP.mrptypestdcode in ('Purchase Order','Workorder')
