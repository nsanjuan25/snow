create or replace view V_ECC_HA_PLANTS(
	PLANTCODE,
	PLANTNAMESHORT,
	PLANTNAMELONG,
	PLANTTYPECODE,
...
----------------------------------------------------------------------------------------------
2021-01-28      Juan Cedeno             Initial Revision
*/
    SELECT 
    ...
    from 
        conformed.ecc_ha_t001w t001w
        LEFT OUTER JOIN  conformed.ecc_ha_t001k t001k
            on  t001w.werks = t001k.bwkey
        LEFT OUTER JOIN  conformed.ecc_ha_t001 t001
            on  t001k.bukrs = t001.bukrs
        LEFT OUTER JOIN conformed.ecc_ha_t399d t399d
            on t001w.werks = t399d.werks
        LEFT OUTER JOIN conformed.ecc_ha_t001a t001a
            on t001K.bukrs = t001a.bukrs
        LEFT OUTER JOIN conformed.scmdb_site site 
            on t001w.werks = site.sitecode  
        LEFT OUTER JOIN SAP.v_CurrenctExchangeRate ce
            on t001.waers = ce.CurrencyCode   
            and ce.datasourcekey = 1
        LEFT OUTER JOIN conformed.v_scmdb_CampusRegion  cr
            ON site.siteid = cr.siteid
        LEFT OUTER JOIN sap.DimCampus DC 
