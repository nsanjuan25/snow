create or replace view V_FACTMRP(
	MRPNUMBER,
	MRPITEM,
	MRPSCENARIO,
	FIXEDVENDOR,
    ...
    FROM 
       (
        SELECT 
            ...
            ESMD.MRPScenario,
            ESMD.FixedVendor,
            DPP.profitcentercode AS PROFITCENTER,
            ...
            CASE 
                WHEN LEFT(ESMH.MRPCurrentAsOFDTS,1) ='0' THEN Left(ESMH.MRPCurrentAsOFDTS,9)
                ELSE Left(ESMH.MRPCurrentAsOFDTS,8)
            ...
            ESMD.MRPElementIndicator ,--CASE WHEN DPST.PurchaseScheduleTypeKey IS NULL THEN ESMD.MRPElementIndicator ELSE '' END MRPElementIndicator,
            ESMD.MRPDocumentTypeCode,
            ESMH.BaseUnitOfMeasureDSCode BaseUnitOfMeasure,
            MRPProcessedFlag,
            LowLevelCode,
            MRPArea,
            MRPPlanningSegment,
       ...
       SELECT 
            ...
            , IFNULL(cancellationleadtime,0) as cancellationleadtime
        FROM
              CONFORMED.V_ECC_HA_MRPDETAIL a ) ESMD 
        INNER JOIN 
		      CONFORMED.V_ECC_HA_MRPHEADER ESMH 
			     ON ESMD.MRPNumber=ESMH.MRPNumber
                 and ESMD.SNAPSHOT_ >= DATEADD(day,0,current_date)
        INNER JOIN 
		      SAP.DimPlants DP 
			     ON TRIM( ESMD.SiteCode)=DP.PlantCode
			    AND DP.DataSourceKey = 1 
        LEFT JOIN 
		      SAP.DimPlants PP 
			     ON TRIM(ESMD.PlanningProductionPlant)=PP.PlantCode
			    AND PP.DataSourceKey = 1 
        INNER JOIN 
		       SAP.DimMaterials DPP 
			     ON (RTRIM(ESMD.SitePartCode)) = DPP.MaterialCode 
                AND DP.PlantKey = DPP.PlantKey
                AND DPP.MaterialKey > 0
		        AND DPP.DataSourceKey = 1 
        LEFT JOIN 
		      SAP.DimMaterials PPP 
			    ON (RTRIM(ESMD.ParentSitePartCode))=PPP.MaterialCode
        ...
        LEFT JOIN (
					...
					WHERE 
						DIMAMLINTERNAL.rn = 1) DA ON 
				            TRIM(ESMD.MPNMaterial) = DA.AMLReference
			...
			LEFT JOIN 
				(
					...
					TRIM(ESMD.FixedVendor)=DS.SupplierCode 
					AND DS.RN = 1
					
			LEFT OUTER JOIN SAP.DimPurchaseScheduleTypes DPST 
				ON TRIM(ESMD.MRPElementIndicator)=DPST.PurchaseScheduleTypeCode
				AND DPST.DataSourceKey = 1 
			LEFT OUTER JOIN SAP.DimMRPTypes MT 
				ON TRIM(ESMD.MRPTypeCode)=MT.MRPTypeCode
...
SELECT 
    ...
    FROM 
        ...
        SELECT 
			...
			, CONCAT(delkz , 
					 ( CASE 
						 WHEN COALESCE((CASE WHEN  delkz ='FE' THEN 'X' ELSE fix01 END),'0')='X' AND COALESCE((CASE WHEN  delkz ='FE' THEN 'X' ELSE fix02 END), '0' ) = 'X' AND TRIM(delkz)  <> 'VJ' 
						 THEN '-XX' 
						 WHEN COALESCE((CASE WHEN  delkz ='FE' THEN 'X' ELSE fix01 END),'0')='X' AND (CASE WHEN  delkz ='FE' THEN 'X' ELSE fix02 END) IS NULL AND TRIM( delkz ) <> 'VJ' 
						 THEN '-X' 
						 WHEN TRIM(delkz) = 'PP' AND TRIM(baart) = 'SAFT' 
						 THEN CONCAT('-' , 'SAFT') 
		...
		FROM
			...
			LEFT JOIN SAP.DimPlants DP ON 
				TRIM( ESMD.SiteCode)=DP.PlantCode
				AND DP.DataSourceKey = ESMD.DataSourceKey 
			LEFT JOIN SAP.DimPlants PP ON 
				TRIM(ESMD.PlanningProductionPlant)=PP.PlantCode
			...
			LEFT JOIN (
						...
						WHERE 
							DIMAMLINTERNAL.rn = 1) DA ON TRIM(ESMD.MPNMaterial) = DA.AMLReference
							AND DA.PlantKey  = DP.PlantKey
...
SELECT 
    ...
    FROM 
        ...
        SELECT 
            ...
            , IFNULL(cancellationleadtime,0) as cancellationleadtime
    FROM
        CONFORMED.V_JPS_HA_MRPDETAIL a ) ESMD 
        INNER JOIN CONFORMED.V_JPS_HA_MRPHEADER ESMH ON 
            ESMD.MRPNumber=ESMH.MRPNumber
            and ESMD.SNAPSHOT_ >= DATEADD(day,0,current_date)
        INNER JOIN SAP.DimPlants DP ON 
            TRIM( ESMD.SiteCode)=DP.PlantCode
			AND DP.DataSourceKey = 5 
        LEFT JOIN SAP.DimPlants PP ON 
            TRIM(ESMD.PlanningProductionPlant)=PP.PlantCode
        ...
        INNER JOIN SAP.DimMaterials DPP ON 
            ...
            AND DP.PlantKey = DPP.PlantKey
            AND DPP.MaterialKey > 0
		  AND DPP.DataSourceKey = 5 
        LEFT JOIN SAP.DimMaterials PPP ON 
            (RTRIM(ESMD.ParentSitePartCode))=PPP.MaterialCode
            AND DP.PlantKey = PPP.PlantKey
			AND PPP.DataSourceKey = 5 
        LEFT JOIN (
					SELECT 
						DIMAMLINTERNAL.AMLKey
                      , DIMAMLINTERNAL.AMLReference
					...
					WHERE 
						DIMAMLINTERNAL.rn = 1) DA ON 
				  RTRIM(ESMD.MPNMaterial) = DA.AMLReference
				  AND DA.PlantKey  = DP.PlantKey
			LEFT JOIN 
				(
					SELECT 
			...
			LEFT OUTER JOIN SAP.DimPurchaseScheduleTypes DPST 
				ON TRIM(ESMD.MRPElementIndicator)=DPST.PurchaseScheduleTypeCode
				AND DPST.DataSourceKey = 5 
			LEFT OUTER JOIN SAP.DimMRPTypes MT 
				ON  TRIM(ESMD.MRPTypeCode)=MT.MRPTypeCode
				AND MT.DataSourceKey = 5 
		) FQ
	WHERE 
		FQ.Row_ID = 1;