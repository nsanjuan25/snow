create or replace view V_PLA_HA_FACTPURCHASEORDERS(
...
SELECT
	  PurchaseDocumentNumber
    , PurchaseDocumentItem
    , ifnull( PURCHASEDOCUMENTTYPEKEY , 0 )   as PURCHASEDOCUMENTTYPEKEY 
...
--	, ifnull(SELLERKEY , 0 ) AS SELLERKEY
	...
	, ifnull( PURCHASEITEMCATEGORYKEY , 0  )  as PURCHASEITEMCATEGORYKEY
	, ifnull( SPECIALSTOCKSKEY , 0 )          as SPECIALSTOCKSKEY
...
FROM
    (
      ...
      FROM
          conformed.PLA_ha_ekko ekko
          INNER JOIN conformed.PLA_ha_ekpo ekpo 
              ...
              (
                  ...
                  from 
                      (
                          ...
                          from 
                              conformed.PLA_ha_eine p 
                              inner join conformed.PLA_ha_eina i
          ...
          LEFT JOIN conformed.PLA_ha_tcurx TCURX 
    ...
    LEFT OUTER JOIN sap.DimPurchaseItemCategories PIC 
        ...
        AND PIC.DataSourceKey = 4
        
    LEFT OUTER JOIN sap.DimSpecialStocks SS 
        on TRIM(pos.SpecialStocksCode) = TRIM(SS.SpecialStocksCode)
        AND SS.DataSourceKey = 4
        
    LEFT OUTER JOIN sap.DimManufacturers M 
        on TRIM(pos.MfrCorporationDSCode) = M.ManufacturerCode
        AND M.DataSourceKey = 4

    LEFT OUTER JOIN sap.DimPaymentTerms AS DPT ON upper(trim(pos.PaymentTerms)) = upper(trim(DPT.PaymentTerms))
        and DPT.datasourcekey = 4
...
*/
    LEFT OUTER JOIN 
        ...
        AND DER.DataSourceKey = 4
    
    LEFT OUTER JOIN sap.DimCurrencyDecimalPlaces AS DCDP 
        ON   DCDP.CurrencyCode = pos.CurrencyCode 
	    AND DCDP.ExponentFactor > 0         
        AND DCDP.DataSourceKey = 4

   -- //LEFT OUTER JOIN (select  DISTINCT rcvprn from  conformed.PLA_ha_EDP13 WHERE mestyp IN ('DELINS', 'ORDCHG', 'ORDERS', 'ZSM') ) e ON pos.SupplierCorporationDSCode = e.rcvprn
