create or replace view V_JPS_DIMSUPPLIERCOMPANIES(
	SUPPLIERCODE,
	CORPORATIONID,
...
-------------------------------------------------------------------------------
2021-09-16	 Jagan P	        InitialVersion
2022-01-19   Carlos Carrillo   Adding data for PaymentTerms and VendorMasterPaymentTerms
...
SELECT 
  ...
  , ''                          AS DUNSNumber 
FROM 
    CONFORMED.JPS_HA_LFA1 AS ESV
    LEFT JOIN    CONFORMED.JPS_HA_LFB1 AS LFB ON ESV.lifnr = LFB.lifnr 
    LEFT JOIN    CONFORMED.JPS_HA_LFM1 AS LFM ON ESV.lifnr = LFM.lifnr 
