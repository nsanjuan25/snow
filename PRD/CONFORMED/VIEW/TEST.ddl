...
FROM
    ...
    INNER JOIN Public.Dimdates DA ON tm1.exportdate = DA.datekey
    INNER JOIN
        ( 
             SELECT 
                  ExportDate 
                , Version 
                , try_to_date ( MIN(TM1.Period ) || '-01' , 'mmmm-yy-dd') as ForecastStartDateKey
            FROM
                conformed.TM1_RPGL_IMDB TM1
            GROUP BY
                  ExportDate 
                , Version 
        )A  ON TM1.ExportDate=A.ExportDate
