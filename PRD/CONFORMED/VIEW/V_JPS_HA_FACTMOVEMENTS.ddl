create or replace view V_JPS_HA_FACTMOVEMENTS(
	MOVEMENTTYPEKEY,
	PLANTKEY,
	MATERIALKEY,
...
----------------------------------------------------------------------------------------------
2021-07-29      Jagan                   Initial Revision
...
SELECT 
  ...
  , ifnull(mseg.blart,'')              as movementdocumenttype
  , ifnull(mseg.mjahr,'')              as movementyear
  , ifnull(mseg.mblnr,'')              as movementdocumentnumber
  , ifnull(mseg.zeile,'')              as movementdocumentitem
  , ifnull(mseg.werks,'')              as sitecode 
  , ifnull(mseg.matnr,'')              as sitepartcode  
  , ifnull(to_date(case when mseg.budat = '00000000' then '19000101' else mseg.budat end , 'YYYYMMDD'),'1900-01-01') as posteddts
  , ifnull(mseg.xblnr,'')              as referencedocumentnumber
  , ifnull(mseg.bwart,'UNDF')          as movementtypecode
  , ifnull(mseg.frbnr,'')              as billoflading
  , ifnull(mseg.bktxt,'')              as movementcomments
  ...
  , ifnull(mseg.usnam,'')              as dscreateuser
  ...
  , ifnull(to_date(case when mseg.cpudt = '00000000' then '19000101' else mseg.cpudt end , 'YYYYMMDD'),'1900-01-01') as transactiondts
  ...
  , ifnull(mseg.anln1,'')             as assetnumber
  , ifnull(mseg.aufnr,'')             as ordernumber
  , ifnull(mseg.kostl,'')             as costcenter 
  , ifnull(mseg.ps_psp_pnr,'')        as wbselementcode 
  , ifnull(mseg.sakto,'')             as glaccount  
  , ifnull(mseg.buzei,'')             as accountinglineitem   
  
  --, LEFT( IFNULL(prps.posid,'') , 50 ) as WBSElement
  , ''                                as WBSElement
  , mseg.FRMW_Extracted_on            as UpdatedOn
FROM 
--   conformed.JPS_HA_MKPF  mkpf  
/*	 conformed.JPS_HA_MATDOC
INNER JOIN 
     conformed.JPS_HA_MATDOC mseg     
        on mkpf.mblnr = mseg.mblnr 
        and mkpf.mjahr = mseg.mjahr */ --- JPS migrated to S4 Hana. So, MKPF will be available using MATDOC
     conformed.JPS_HA_MATDOC mseg     		
LEFT OUTER JOIN 
     conformed.JPS_HA_LFA1 lfa1
        on mseg.lifnr = lfa1.lifnr    
LEFT OUTER JOIN 
     sap.DimMovementTypes MT
        on ifnull(mseg.bwart,'UNDF') = TRIM( MT.MovementTypeCode)
...
LEFT OUTER JOIN 
     sap.DimReasonTypes R
        ...
        AND R.DataSourceKey = 5
LEFT OUTER JOIN 
     sap.DimSuppliers S 
        on IFNULL( mseg.lifnr,'UNDF') = S.SupplierCode
        AND IFNULL( lfa1.ktokk , 'UNDF')   = S.SupplierTypeCode
        AND S.DataSourceKey = 5
LEFT OUTER JOIN 
     sap.DimExchangeRates ER
        ON ifnull(mseg.waers,'') = ER.currencycode
        and ER.ToCurrencyCode = 'USD'
        and ifnull(to_date(mseg.cpudt, 'YYYYMMDD'),'1900-01-01') = ER.EffectiveFromDateKey
        and ER.DataSourceKey = 5
LEFT OUTER JOIN 
     sap.DimCurrencyDecimalPlaces  DCDP
        ON DCDP.CurrencyCode = ifnull(mseg.waers,'')
        and DCDP.DataSourceKey = 5
        and DCDP.ExponentFactor > 0
 /*   
    LEFT OUTER JOIN conformed.jps_ha_prps prps 
        ON ifnull(mseg.ps_psp_pnr,'') = prps.pspnr*/  -- commented by Jagan
;