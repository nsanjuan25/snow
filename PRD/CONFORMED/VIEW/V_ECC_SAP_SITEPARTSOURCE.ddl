create or replace view V_ECC_SAP_SITEPARTSOURCE(
	DATASOURCEKEY,
	PLANTCODE,
	MATERIALCODE,
...
(
   ...
   FROM 
	(
      ...
      FROM 
		CONFORMED.ECC_HA_EORD a 
		INNER JOIN CONFORMED.ECC_HA_LFA1 r ON a.lifnr = r.lifnr 
		LEFT OUTER JOIN CONFORMED.ECC_HA_MARC c ON a.matnr = c.matnr 
			AND a.werks = c.werks 
		 LEFT OUTER JOIN CONFORMED.ECC_HA_MARA d ON a.matnr = d.matnr 
      WHERE 
		a.lifnr IS NOT NULL 
	...
	(
		...
		FROM 
			 CONFORMED.ECC_HA_EQUK p 
			 INNER JOIN CONFORMED.ECC_HA_EQUP q ON p.qunum = q.qunum 
			 LEFT OUTER JOIN CONFORMED.ECC_HA_EORD a ON p.werks = a.werks 
				AND p.matnr = a.matnr 
			 INNER JOIN CONFORMED.ECC_HA_LFA1 r ON q.lifnr = r.lifnr 
			 LEFT OUTER JOIN CONFORMED.BW_HA_PLAF s ON p.matnr = s.matnr 
				AND p.werks = s.plwrk 
				AND q.lifnr = s.flief 
				AND p.qunum = s.qunum 
			 LEFT OUTER JOIN CONFORMED.ECC_HA_RDPR t ON p.werks = t.werks 
				AND q.rdprf = t.rdprf 
         WHERE 
			q.lifnr IS NOT NULL
        ) query2 
